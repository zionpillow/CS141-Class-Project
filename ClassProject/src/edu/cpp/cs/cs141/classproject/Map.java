...
public class Map implements Serializable {
	...
	/**
	 * This field is used in keeping track of the player's column prior to
	 * moving. This field is used in order to replace the player back in their
	 * original position if the player is unable to move. In addition, this
	 * field is also partly used in determining the AI movement of the enemy in
	 * the method, {@link #moveAI()}.
	 */
	 ...
	 * {@link #movePlayer(edu.cpp.cs.cs141.classproject.UI.direction)} and is
	 * used to determine where the player is at all times. This is useful for
	 * when the player {
	 * {@link #look(edu.cpp.cs.cs141.classproject.UI.direction, int)}s or
	 * {@link #shoot(edu.cpp.cs.cs141.classproject.UI.direction)}s.
	 */
	private int playerRow;

	/**
	 * This field represents the player's current column. This field can be
	 * altered when the player moves through the method,
	 * {@link #movePlayer(edu.cpp.cs.cs141.classproject.UI.direction)} and is
	 * used to determine where the player is at all times. This is useful for
	 * when the player {
	 * {@link #look(edu.cpp.cs.cs141.classproject.UI.direction, int)}s or
	...
	/**
	 * This field represents the type of the last item that the player picked
	 * up. If this field is not {@code null}, then the item will be resolved by
	 * the method,
	 * {@link #resolveItem(edu.cpp.cs.cs141.classproject.Item.itemType)}, and be
	 * reset to null by the method, {@link #resetLastItem()}.
	 */
	...
	/**
	 * This field represents whether or not the game is in "debug mode". This
	 * field is initialized when a game is first initialized by the
	 * {@link GameEngine} through the method,
	 * {@link #initialize(boolean, boolean, boolean, int)}} and can never be
	 * altered.
	 */
	private boolean debug;

	/**
	 * This field represents whether or not the game is in "hard mode". This
	 * field is initialized when a game is first initialized by the
	 * {@link GameEngine} through the method,
	 * {@link #initialize(boolean, boolean, boolean, int)}} and can never be
	 * altered.
	...
	/**
	 * This field represents whether or not the game is in "God mode". This
	 * field is initialized when a game is first initialized by the
	 * {@link GameEngine} through the method,
	 * {@link #initialize(boolean, boolean, boolean, int)}} and can never be
	 * altered.
	 ...
	 * initialized to 1 when a game is first initialized by the
	 * {@link GameEngine} through the method,
	 * {@link #initialize(boolean, boolean, boolean, int)} and is increased in
	 * increments of 1 when the method, {@link #nextLevel(int)}, is called.
	 */
	private int level;
	 ...
	 * to this field when the player decides to move onto the next level by
	 * calling the method, {@link #nextLevel(int)}.
	 */
	 ...
	 * specific level that the player is currently on. This field is only
	 * increased when the player checks a room in the method,
	 * {@link #movePlayer(edu.cpp.cs.cs141.classproject.UI.direction)}.
	 */
	 ...
	 * specific level that the player is currently on. This field is only
	 * increased when the player obtains an item in the method,
	 * {@link #movePlayer(edu.cpp.cs.cs141.classproject.UI.direction)}.
	 */
	 ...
	 * specific level that the player is currently on. This field is only
	 * increased when the player kills an enemy in the method,
	 ...
	 * prior to the current level. The field {@link #enemiesKilled} is only
	 * added to this field when the player decides to move onto the next level
	 * by calling the method, {@link #nextLevel(int)}.
	 */
	 ...
	 * when the {@link #tallyScore(boolean)} method is called based on the
	 * different circumstances.
	 */
	private int score;

	/**
	 * This field represents the score multiplier associated with each mode.
	 * This field is initialized in the method,
	 * {@link #initialize(boolean, boolean, boolean, int)}, based on the mode
	 * that the player is playing in and can never be changed.
	 */
	private double modeMultiplier;

	/**
	 * This field represents the score multiplier associated with the current
	 * level. This field is initialized as 1 in the method,
	 * {@link #initialize(boolean, boolean, boolean, int)}, and is increased by
	 * 0.5 for each level after level 1 through the method,
	 * {@link #nextLevel(int)}.
	 */
	private double levelMultiplier;

	/**
	 * This constructor method initializes the random object assigned to the
	 * field, {@link #rand}, in order to be used as a random number generated
	 * for random events.
	 */
	public Map() {
		rand = new Random();
	...
	/**
	 * This method first assigns This method Initializes and places all
	 * {@code Entity} objects necessary within the game map ((0, 0) being the
	 * top left corner and (8, 8) being the bottom right corner).
	 * 
	 * @param debug
	 ...
	 * {@link #hardMode}, and {@link #godMode} based on the parameters given. In
	 * addition, this method also sets proper values to each of the fields
	 ...
	 * right corner), where the number of enemies placed is dictated by the
	 * level.
	 * 
	 * @param debug
	 *            {@code true} if in debug mode, {@code false} otherwise
	 * @param hardMode
	 *            {@code true} if in hard mode, {@code false} otherwise
	 * @param godMode
	 *            {@code true} if in God mode, {@code false} otherwise
	 * @param maxAmmo
	 *            an int representing the maximum ammo of the player based on
	 *            the current magazine upgrade of the {@link Shop} in the
	 *            {@link GameEngine}
	 */
	 ...
	 * and multipliers accordingly. Then, this method creates a new game world
	 * with randomly placed entities and takes the player from the previous game
	 * world and re-places that same player object in the same starting position
	 * as the previous game world.
	 * 
	 * @param gainLife
	 *            an int representing the number of lives gained when moving
	 *            onto the next level, which is determined by the current health
	 *            upgrade of the {@link Shop} through the {@link GameEngine}
	 */
	 ...
	 * 
	 * @param numOfEnemies
	 *            the number of enemies to be randomly placed
	 */
	 ...
	 * will place the given entity in that position. This method also ensures
	 * that no entity, whether enemy or item, can be placed within the spawning
	 * area of the player, the 3 by 3 block area at the bottom left of the
	 * world.
	 * 
	 * @param entity
	 ...
	 * The player initially can only look 2 tiles in front of them. However,
	 * this feature can be upgraded in the {@link Shop} through the
	 * {@link GameEngine}. This method then returns an integer value
	 * representing the distance from the player to the first enemy in their
	 * line of sight that is within the vision limits. If the player does not
	 * see any enemies, is looking at a wall, or is looking at a room, this
	 * method will return 0.
	 * 
	 * @param dir
	 *            the direction the player wants to look at
	 * @param distance
	 *            an int representing how far the player's vision can see
	 ...
	 * 
	 * @param dir
	 *            the direction the player wants to shoot
			...
			for (int i = playerColumn; i >= 0; --i) {
				if (gameMap[playerRow][i] != null) {
					if (gameMap[playerRow][i].getEntityType() == Entity.entityType.ENEMY) {
						...
						killed = true;
						hasCollided = true;
					} else if (gameMap[playerRow][i].getEntityType() == Entity.entityType.ROOM)
						hasCollided = true;
				}
	 ...
	 * the result from the attempt to move will be returned. From this
	 * information, the {@link GameEngine} will be able to tell whether the
	 * player has moved or hasn't moved and the reason for the action that's
	 * been made.
	 * 
	 * @param dir
	 *            the direction the player wants to move
	 * @return the type of result from the attempt to move in the given
	 *         direction
	 */
	 ...
	 * player's starting position, and calls the method,
	 * {@link #playerLostLife()}, when the player is on a tile next to a ninja
	 * at the start of the enemies' turn.
	 */
	...
	/**
	 * This method reduces the player's life and also repositions enemies if
	 * there exists any number of enemies within the player's spawn area, the 3
	 * by 3 block area at the bottom left of the world.
	 */
	...
	/**
	 * This method sets the player's max ammo when the game is initialized based
	 * on the current magazine upgrade in the {@link Shop}. This method is only
	 * called once in the method,
	 * {@link #initialize(boolean, boolean, boolean, int)}, and the player's max
	 * ammo is never changed afterwards.
	 * 
	 * @param maxAmmo
	 *            an int that represents the player's designated max ammo based
	 ...
	 * 
	 * @param type
	 *            the type of item picked up by the player
	 */
	...
	/**
	 * This method ensures that an item can only be used once, and is called
	 * after resolving an item's use.
	 */
	...
	/**
	 * This method reduces the number of turns the player has left to be
	 * invincible when the player has picked up an invincibility item and has
	 * turns of invincibility remaining.
	 */
	...
	/**
	 * This method allows every enemy on the map to scan the four adjacent
	 * spaces around them. If a player is on a space adjacent to any ninja on
	 * the map, this method will return {@code true}, otherwise this method will
	 * return {@code false}.
	 * 
	 ...
	 * the ninja in that direction. If no directions are available, then the
	 * ninja will remain in the same spot.
	 */
	 ...
	 * if one or two directions were to be chosen so that the ninja approaches
	 * the player, and compares those directions to the available directions to
	 * see which possible directions the ninja would move in if it was to seek
	 * the player. Then, the choice between whether the ninja seeks the player
	 * or is moved randomly is randomized, with the chance of seeking increased
	 * the farther away the ninja is from the player. If seeking fails, then a
	 * direction is randomly chosen from the available directions. If seeking
	 * succeeds but there are no possible directions the ninja could move in
	 * order to seek the player, then the ninja is moved in whatever direction
	 * is available. If there are no directions available, then the ninja will
	...
	/**
	 * This method adds the current level's score onto the overall score. The
	 * score calculated differs on whether the condition is that the player
	 * cleared the level, in which extra points are added, or whether the
	 * condition is that the player lost before the level was cleared, in which
	 * the level clear points are not added.
	 * 
	 * @param win
	 *            {@code true} if the player cleared the level, {@code false}
	 *            otherwise
	 */
	...
	/**
	 * This method retrieves the type of item that was last picked up by the
	 * player.
	 * 
	...
	/**
	 * This method calculates the points given for the number of lives the
	...
	/**
	 * This method calculates the points given for the number of rooms the
	 * player checked.
	 * 
	 * @return an int that represents the score for the number of rooms the
	 *         player checked
	 */
	...
	/**
	 * This method calculates the points given for the number of items the
	 * player obtained.
	 * 
	 * @return an int that represents the score for the number of items the
	 *         player obtained
	 */
	...
	/**
	 * This method calculates the points given for the number of enemies the
	 * player killed.
	 * 
	 * @return and int that represents the score for the number of enemies the
	 *         player killed
	 */
	...
	public int getLevelScore() {
		return (getNumOfTurnsScore() + getLivesRemainingScore() + getRoomsCheckedScore() + getItemPickupsScore()
				+ getEnemiesKilledScore());
	}

	/**
	 * This method returns the value of the field {@link #totalNumOfTurns} plus
	 * the value of the field {@link #numOfTurns} in order to get the overall
	...
	/**
	 * This method returns the value of the field {@link #totalRoomsChecked}
	 * plus the value of the field {@link #roomsChecked} in order to get the
	 * overall number of rooms the player has checked.
	 * 
	 ...
	 * plus the value of the field {@link #itemPickups} in order to get the
	 * overall number of items the player has obtained.
	 * 
	 ...
	 * {@link #getTotalEnemiesKilled()} plus the value of the field
	 * {@link #enemiesKilled} in order to get the overall number of enemies the
	 * player has killed.
	 * 
	 ...
	 * method,
	 * {@link UI#printGameOver(int, int, int, int, int, int, int, String[])}.
	 * 
