...
public class Map implements Serializable {
	...
	public boolean look(UI.direction dir){
		...
		case UP:
			for(int i = (playerRow-2) ; i < playerRow ; ++i) {
				if(i >= 0 && gameMap[i][playerColumn] != null && gameMap[i][playerColumn].getEntityType() == Entity.entityType.ENEMY)
					detectedEnemy = true;
			}
			break;
		case DOWN:
			for(int i = (playerRow+2) ; i > playerRow ; --i) {
				if(i <= gameMap.length && gameMap[i][playerColumn] != null && gameMap[i][playerColumn].getEntityType() == Entity.entityType.ENEMY)
					detectedEnemy = true;
			}
			break;
		case LEFT:
			for(int i = (playerColumn-2) ; i < playerColumn ; ++i) {
				if(i >= 0 && gameMap[playerRow][i] != null && gameMap[playerRow][i].getEntityType() == Entity.entityType.ENEMY)
					detectedEnemy = true;
			}
			break;
		case RIGHT:
			for(int i = (playerColumn+2) ; i > playerColumn ; --i) {
				if(i <= gameMap[0].length && gameMap[playerRow][i] != null && gameMap[playerRow][i].getEntityType() == Entity.entityType.ENEMY)
					detectedEnemy = true;
			}
			break;
		}
	...
	public boolean enemyScan() {
		...
		for(int i = 0 ; i < gameMap.length ; i++) {
			for(int j = 0 ; j < gameMap[0].length ; j++) {
				if(gameMap[i][j] != null && gameMap[i][j].getEntityType() == Entity.entityType.ENEMY) {
					if(gameMap[i-1][j] != null && gameMap[i-1][j].getEntityType() == Entity.entityType.PLAYER)
						nextToPlayer = true;
					else if(gameMap[i+1][j] != null && gameMap[i+1][j].getEntityType() == Entity.entityType.PLAYER)
						nextToPlayer = true;
					else if(gameMap[i][j-1] != null && gameMap[i][j-1].getEntityType() == Entity.entityType.PLAYER)
						nextToPlayer = true;
					else if(gameMap[i][j+1] != null && gameMap[i][j+1].getEntityType() == Entity.entityType.PLAYER)
						nextToPlayer = true;
				}
